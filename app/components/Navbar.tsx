...
import { Shield, Lock, LogOut, Menu, X, Loader2 } from 'lucide-react';
...
export default function Navbar() {
  const { isAdmin, logout, login, loading } = useAuth();
  ...
  return (
    <>
      <nav ref={navRef} className="fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-[#fafaf9] w-screen"
        style={{ boxShadow: scrolled ? '0px 14px 50px -1px rgba(87,87,87,0.3)' : 'none' }}>
        <div className="px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-12">
            {navbar.links.map(link => (
              <a key={link.href} href={link.href}
                ...
                style={{ fontFamily: 'var(--font-body)', color: 'var(--text-muted)', letterSpacing: '0.02em' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--cyan)'; (e.currentTarget as HTMLElement).style.background = 'rgba(0,212,255,0.06)'; }}
          ...
          <div className="hidden md:flex items-center">
            {isAdmin ? (
              <div className="flex items-center gap-2">
                <span className="tag" style={{ color: 'var(--green)', borderColor: 'var(--green)', fontFamily: 'var(--font-mono)' }}>ADMIN</span>
              ...
              <button onClick={() => setShowLoginModal(true)}
                ...
                style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', letterSpacing: '0.05em', color: 'var(--cyan)', borderColor: 'var(--border-bright)', background: 'rgba(0,212,255,0.04)' }}
          ...
          {/* Mobile toggle */}
          <button className="md:hidden" style={{ color: 'var(--cyan)' }} onClick={() => setMobileOpen(!mobileOpen)}>
          ...
          <div className="md:hidden px-6 pb-6 pt-2 flex flex-col gap-2">
            {navbar.links.map(link => (
              <a key={link.href} href={link.href} onClick={() => setMobileOpen(false)}
                className="py-2 text-sm" style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-body)' }}>
                {link.label}
              </a>
            ))}
            {!isAdmin && (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-sm p-8 rounded-lg"
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.2rem' }}>Admin Access</h3>
            </div>
            ...
            <form onSubmit={handleLogin} className="flex flex-col gap-4">
              <div>
                ...
                />
                {loginError && (
                  <p className="mt-1 text-xs" style={{ color: 'var(--red)', fontFamily: 'var(--font-mono)' }}>
                    {loginError}
                  </p>
                )}
              </div>
              ...
              <button type="submit" disabled={loading}
                className="w-full py-3 rounded text-sm font-semibold transition-all flex items-center justify-center gap-2"
                style={{ background: 'var(--cyan)', color: 'var(--bg)', fontFamily: 'var(--font-mono)', letterSpacing: '0.08em', opacity: loading ? 0.7 : 1 }}>
                {loading ? <><Loader2 size={14} className="animate-spin" /> CHECKING...</> : 'ACCESS GRANTED'}
              </button>

              <p className="text-center text-xs" style={{ color: 'var(--text-dim)', fontFamily: 'var(--font-mono)' }}>
                Password managed via{' '}
                <span style={{ color: 'var(--cyan)' }}>Vercel KV</span>
              </p>
            </form>
          </div>
        </div>
